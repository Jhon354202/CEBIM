...
public class TaskReceivedFragment extends BaseFragment {
    ...
    private List<SynergyTaskEntity> receivedTaskList = new ArrayList<>();//获取回调接口中返回的
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_task_received, container, false);
    ...
    private void initView() {
        //RecyclerView逻辑
        recyclerView = (RecyclerView) view.findViewById(R.id.received_task_recyclerview);
        LinearLayoutManager layoutManager = new LinearLayoutManager(mActivity);
        recyclerView.setLayoutManager(layoutManager);

        //SwipeRefreshLayout下拉刷新的逻辑
        swipeRefresh = (SwipeRefreshLayout) view.findViewById(R.id.received_swip_refresh);
        ...
        listIsnull = view.findViewById(R.id.rl_received_list_is_null);
    }
                    ...
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            if ((UserIds != null && UserIds.contains(userName)) ||
                                ...
                                //数据源
                                receivedTaskList.add(synergyTaskEntity);
                            }
                        ...
                        mActivity.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (receivedTaskList == null || receivedTaskList.size() == 0) {
                                    ...
                                    if (!isPull) {
                                        adapter = new TaskAdapter(mActivity, receivedTaskList,"TaskReceivedFragment");
                                        recyclerView.addItemDecoration(new TaskReceiverDividerItemDecoration(MyApplication.getContext()));
                                        ...
                                        if (receivedTaskList != null && receivedTaskList.size() > 0 && adapter != null) {
                                            adapter.notifyDataSetChanged();
                                            if (swipeRefresh.isRefreshing()) {
                                                swipeRefresh.setRefreshing(false);
                                            }
                                        } else if (receivedTaskList != null && receivedTaskList.size() > 0 && adapter == null) {
                                            adapter = new TaskAdapter(mActivity, receivedTaskList,"TaskReceivedFragment");
                                            recyclerView.addItemDecoration(new TaskReceiverDividerItemDecoration(MyApplication.getContext()));
                                            recyclerView.setAdapter(adapter);
                                            if (swipeRefresh.isRefreshing()) {
    ...
    class TaskReceiverDividerItemDecoration extends Y_DividerItemDecoration {

        public TaskReceiverDividerItemDecoration(Context context) {
            super(context);
        }
