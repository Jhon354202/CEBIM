 ...
 * 协同管理——问题——已延期Fragment
 */
public class QuestionDelayFragment extends BaseFragment {
    ...
    private List<QuestionModel> delayQuestionList = new ArrayList<>();//获取回调接口中返回的
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_question_delay, container, false);
        //注册EventBus
    ...
    private void initView() {
        //RecyclerView逻辑
        recyclerView = (RecyclerView) view.findViewById(R.id.delay_recyclerview);
        LinearLayoutManager layoutManager = new LinearLayoutManager(mActivity);
        recyclerView.setLayoutManager(layoutManager);

        //SwipeRefreshLayout下拉刷新的逻辑
        swipeRefresh = (SwipeRefreshLayout) view.findViewById(R.id.delay_swip_refresh);
        ...
        listIsnull=view.findViewById(R.id.rl_delay_list_is_null);
    }
                ...
                if (response.code() == 200) {
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            //数据源
                            delayQuestionList.add(questionModel);
                        }

                        for (int i = 0; i < delayQuestionList.size(); i++) {
...
//                            }
                            ...
                            if (delayQuestionList.get(i).getPictures() != null && (!delayQuestionList.get(i).getPictures().equals("null")) &&
                                    (!delayQuestionList.get(i).getPictures().equals("[]"))) {
                                //解析字符串
                                String[] arr = delayQuestionList.get(i).getPictures()
                                        .replace("{", "").replace("}", "")
                                        .replace("[", "").replace("]", "").split(",");

                                String id = arr[1].split(":")[1].replace("\"", "").replace("\"", "").toString();
                                String picUrl = AppConst.innerIp + "/api/AnnexFile/" + id;
                                delayQuestionList.get(i).setFirstFrame(picUrl);
                            }
                        ...
                        mActivity.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (!isPull) {
                                    adapter = new QuestionAdapter(MyApplication.getContext(), delayQuestionList);
                                    ...
                                    if (delayQuestionList != null && delayQuestionList.size() > 0 && adapter != null) {
                                        adapter.notifyDataSetChanged();
                                        if (swipeRefresh.isRefreshing()) {
                                            swipeRefresh.setRefreshing(false);
                                        }
                                    } else if (delayQuestionList != null && delayQuestionList.size() > 0 && adapter == null) {
                                        adapter = new QuestionAdapter(MyApplication.getContext(), delayQuestionList);
                                        recyclerView.addItemDecoration(new DividerItemDecoration(mActivity, DividerItemDecoration.VERTICAL));
                                        recyclerView.setAdapter(adapter);
                                        if (swipeRefresh.isRefreshing()) {
