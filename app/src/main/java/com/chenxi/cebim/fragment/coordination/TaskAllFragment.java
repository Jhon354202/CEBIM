...
public class TaskAllFragment extends BaseFragment {
    ...
    private List<SynergyTaskEntity> allTaskList = new ArrayList<>();//获取回调接口中返回的
    ...
    private void initView() {
        //RecyclerView逻辑
        recyclerView = (RecyclerView) view.findViewById(R.id.all_task_recyclerview);
        ...
        listIsnull = view.findViewById(R.id.rl_all_list_is_null);
    }
                ...
                if (response.code() == 200) {
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            //数据源
                            allTaskList.add(synergyTaskEntity);
                        }
                        ...
                        mActivity.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (!isPull) {
                                    adapter = new TaskAdapter(mActivity, allTaskList,"TaskAllFragment");
//                                    recyclerView.addItemDecoration(new DividerItemDecoration(mActivity,
//                                            DividerItemDecoration.VERTICAL));
                                    recyclerView.addItemDecoration(new MyDividerItemDecoration(MyApplication.getContext()));
                                    ...
                                    if (allTaskList != null && allTaskList.size() > 0 && adapter != null) {
                                        adapter.notifyDataSetChanged();
                                        if (swipeRefresh.isRefreshing()) {
                                            swipeRefresh.setRefreshing(false);
                                        }
                                    } else if (allTaskList != null && allTaskList.size() > 0 && adapter == null) {
                                        adapter = new TaskAdapter(mActivity, allTaskList,"TaskAllFragment");
                                        recyclerView.addItemDecoration(new MyDividerItemDecoration(MyApplication.getContext()));
                                        recyclerView.setAdapter(adapter);
                                        if (swipeRefresh.isRefreshing()) {
    ...
    class MyDividerItemDecoration extends Y_DividerItemDecoration {
