...
public class CategoryActivity extends BaseActivity {
    ...
    private void initCategory(final int isRefresh) {
        Request request = new Request.Builder()
                .url(AppConst.innerIp + "/api/"+SPUtils.getInstance().getInt("projectID")+"/SynergyQuestion/Categories")
                .build();
                ...
                if(response.code()==200){
                    try {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (isRefresh == 0) {
                                    showStringAdapter = new ShowStringAdapter(list,"CategoryActivity");
    ...
    public void Event(NewQuestionDelEven newQuestionDelEven) {
        ...
        if(newQuestionDelEven.getInfo().equals("CategoryActivityfinish")){//从ShowStringAdapter返回,用于finish本界面
            finish();
        }
    }
