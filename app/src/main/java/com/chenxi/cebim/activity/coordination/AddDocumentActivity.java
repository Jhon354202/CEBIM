...
import com.chenxi.cebim.adapter.AddDocumentAdapter;
...
public class AddDocumentActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SPUtils.getInstance().put(SPUtils.getInstance().getInt("projectID") + "新建问题", documentFileString);
    ...
    private void initView() {
        swipeRefresh = findViewById(R.id.document_swip_refresh);
        ...
        listIsNull = findViewById(R.id.rl_document_list_is_null);
        ...
        //底部确认按钮
        bottomSure = findViewById(R.id.tv_bottom_sure);
        bottomSure.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //EvenBus返回被选项目的fileID给NewQuestion
                if (from.equals("NewQuestion")) {
                    EventBus.getDefault().post(new NewQuestionDelEven("附件FileID@#@#@#" + SPUtils.getInstance().getString(projectID + "新建问题").replace(",,", ",")));
                } else if (from.equals("ProblemDetail")) {
                    Intent intent = new Intent(AddDocumentActivity.this,
                            QuestionResponseActivity.class);
                    intent.putExtra("documentFileString", SPUtils.getInstance().getString(projectID + "新建问题").replace(",,", ","));
                    intent.putExtra("from", "AddDocumentActivity");
                    startActivity(intent);
                } else if (from.equals("QuestionResponseActivity")) {
                    EventBus.getDefault().post(new NewQuestionDelEven("QuestionResponseActivity@#@#@#" + SPUtils.getInstance().getString(projectID + "新建问题").replace(",,", ",")));
                }
...
//        SPUtils.getInstance().put(projectID + "新建问题",documentFileString);
    ...
    private void getData(final int isRefresh, final int isCheck, final Boolean isAllChoosed) {//isRefresh是否为刷新，isChoose是否为选择模式，1为是，0为否
        ...
        Request dirRequest;
        String id1=AppConst.innerIp + "/api/" + projectID + "/EngineeringData/Class?where=ParentClassID=null and Recycle=false";
        String id2=AppConst.innerIp + "/api/" + projectID + "/EngineeringData/Class?where=ParentClassID=" + classID + " and Recycle=false";
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (tempList.size() == 0 || tempList == null) {
                                ...
                                if (isRefresh == 0) {
                                    adapter = new AddDocumentAdapter(MyApplication.getContext(), showList, projectID, documentFileString,from);
                                    adapter.isShowCheckBox(true);//显示选择框
                                    recyclerView.setAdapter(adapter);

                                    boolean hasAllNoChecked = false;//是否有被选中的，如果有，则显示确定和取消按钮，否则不显示
                                    for (int i = 0; i < adapter.getList().size(); i++) {
                                        if (adapter.getList().get(i).getChecked()) {
                                            hasAllNoChecked = true;
                                            break;
                                        }
                                    }
                                    if (hasAllNoChecked) {
                                    ...
                                    if (showList != null && showList.size() > 0 && adapter != null) {
                                        ...
                                        adapter = new AddDocumentAdapter(AddDocumentActivity.this, showList, projectID, documentFileString,from);
                                        adapter.isShowCheckBox(true);//显示选择框
                                        recyclerView.addItemDecoration(new DividerItemDecoration(AddDocumentActivity.this, DividerItemDecoration.VERTICAL));
                                        recyclerView.setAdapter(adapter);
                                        if (swipeRefresh.isRefreshing()) {
                                            swipeRefresh.setRefreshing(false);
                                        }
