...
public class AtActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        atListStr = intent.getStringExtra("atListStr");
        from = intent.getStringExtra("from");//接受传递过来数据的类名

        initView();
        initAt(0);
    }

    private void initAt(final int isRefresh) {
                ...
                if (response.code() == 200) {
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            boolean isChecked;
                            if (atListStr != "" && atListStr.contains(UserName)) {
                                isChecked = true;
                            } else {
                                isChecked = false;
                            }
    ...
    private void initView() {
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(AtActivity.this);
            ...
            public void onClick(View view) {
                ...
                if (hasChecked) {
                    ...
                    if (from.equals("NewQuestion")) {
                        EventBus.getDefault().post(new NewQuestionDelEven("AtActivity@#@#@#" + atName));//传值到NewQuestion
                    } else if (from.equals("ProblemDetail")) {
                        Intent intent = new Intent(AtActivity.this, QuestionResponseActivity.class);
                        intent.putExtra("atListStrForResponse", atName);
                        intent.putExtra("from", "at");
                        startActivity(intent);
                    } else if (from.equals("QuestionResponseActivity")) {
                        Intent intent = new Intent();
                        intent.putExtra("data_return", atName);
...
//                QuestionResponseActivity.atListStrForResponse=sb.toString().substring(0,sb.length()-1);
//                EventBus.getDefault().post(new NewQuestionResponseEvenBusModel("At字符串获取成功"));//传值到NewQuestion
                    finish();
                } else {
