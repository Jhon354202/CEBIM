...
import com.chenxi.cebim.activity.coordination.TaskFeedBackAddDocumentActivity;
...
import static com.chenxi.cebim.activity.coordination.TaskFeedBackActivity.feedBackdocumentString;

public class TaskFeedBackAddDocumnetAdapter extends RecyclerView.Adapter<TaskFeedBackAddDocumnetAdapter.ViewHolder> {
     ...
     */
    public TaskFeedBackAddDocumnetAdapter(Context context, ArrayList<TbFileShowmodel> dataFileList, int projectID, String documentFileString, String from) {

        mDataFileList = dataFileList;
        mProjectID = projectID;
        mDocumentFileString = documentFileString;
        mTempDocumentFileString=feedBackdocumentString;
        mFrom=from;
    };

    public String getDocumentString() {
        if (feedBackdocumentString == null) {
            feedBackdocumentString = "";
        }else{
            feedBackdocumentString=mTempDocumentFileString;
        }
        return feedBackdocumentString;
    }
    ...
    @NonNull
    @Override
    public TaskFeedBackAddDocumnetAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, final int position) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.data_file_item, viewGroup, false);

        final TaskFeedBackAddDocumnetAdapter.ViewHolder holder = new TaskFeedBackAddDocumnetAdapter.ViewHolder(view);
            ...
            public void onClick(View view) {
                ...
                if (isShowCheckBox && mDataFileList.get(position).getFileType().equals("file")) {//在checkBox显示的情况下，点击recyclerView的Item会选中checkBox
                    if (holder.checkBox.isChecked()) {
                        ...
                        //把选中项从feedBackdocumentString中去掉
...
//                        //把选中项从feedBackdocumentString中去掉
//                        feedBackdocumentString=feedBackdocumentString.replace(mDataFileList.get(position).getFileName()+"☆"
//                                +mDataFileList.get(position).getFileID(),"");
...
//                        }
                    ...
                    Intent intent = new Intent(view.getContext(), TaskFeedBackAddDocumentActivity.class);
                    ...
                    intent.putExtra("documentFileString", feedBackdocumentString);
    ...
    public void onBindViewHolder(TaskFeedBackAddDocumnetAdapter.ViewHolder holder, int position) {
        ...
        if (isShowCheckBox && mDataFileList.get(position).getFileType().equals("dir")) {
            ...
            if (feedBackdocumentString.contains("" + tbFileShowmodel.getFileID())
                    &&feedBackdocumentString.contains("" + tbFileShowmodel.getFileName())) {
                holder.checkBox.setChecked(true);
                mDataFileList.get(position).setChecked(true);
                EventBus.getDefault().post(new IsShowDocumentSureBtn("有选中Item"));//通知TaskPublishAddDocumentActivity弹出取消和确定按钮
            } else {
