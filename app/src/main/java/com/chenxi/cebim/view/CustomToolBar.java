...
public class CustomToolBar extends LinearLayout {
    ...
    private Boolean isFisrtRightBtnVisible;
    private int firstRightResId;

    private Boolean isSecondRightBtnVisible;
    private int secondRightResId;
    ...
    public void initView(AttributeSet attrs){
        ...
        /**-------------获取右边第一个按钮属性------------*/
        isFisrtRightBtnVisible = typedArray.getBoolean(R.styleable.CustomToolBar_first_right_btn_visible, false);
        firstRightResId = typedArray.getResourceId(R.styleable.CustomToolBar_first_right_btn_src, -1);
        /**-------------获取右边第二个按钮属性------------*/
        isSecondRightBtnVisible = typedArray.getBoolean(R.styleable.CustomToolBar_second_right_btn_visible, false);
        secondRightResId = typedArray.getResourceId(R.styleable.CustomToolBar_second_right_btn_src, -1);
        /**-------------获取右边文本属性------------*/
        ...
        ImageView firstRightIv = (ImageView)barLayoutView.findViewById(R.id.toolbar_first_right_iv);
        ImageView secondRightIv = (ImageView)barLayoutView.findViewById(R.id.toolbar_second_right_iv);
        ...
        if(isFisrtRightBtnVisible){
            firstRightIv.setVisibility(VISIBLE);
        }
        if(isSecondRightBtnVisible){
            secondRightIv.setVisibility(VISIBLE);
        }
        if(isRightTvVisible){
        ...
        if(firstRightResId != -1){
            firstRightIv.setBackgroundResource(firstRightResId);
        }

        if(secondRightResId != -1){
            secondRightIv.setBackgroundResource(secondRightResId);
        }
        if(backgroundResId != -1){
            barRlyt.setBackgroundColor(getResources().getColor(R.color.title_background_colr));
        }
