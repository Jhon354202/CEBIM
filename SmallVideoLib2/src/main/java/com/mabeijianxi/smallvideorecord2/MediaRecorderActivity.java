...
public class MediaRecorderActivity extends Activity implements
     ...
     * 录制最长时间
     */
    private int RECORD_TIME_MAX = 6 * 10000;
    /**
     * 刷新进度条
     */
     ...
     * 拍摄按钮
     */
    private TextView mRecordController, tv_record_delete, tv_record_choosed;

    /**
     * 底部条
     */
    private RelativeLayout mBottomLayout, mPlayBottomLayout;
    /**
     * 摄像头数据显示画布
     */
    private SurfaceView mSurfaceView, vedioPlay;
    /**
     * 录制进度
     */
...
//        context.startActivity(new Intent(context, MediaRecorderActivity.class).putExtra(OVER_ACTIVITY_NAME, overGOActivityName).putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig));
        context.startActivityForResult(new Intent(context, MediaRecorderActivity.class).putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig), 214);
    }
    ...
    private void loadViews() {
        ...
        vedioPlay = (SurfaceView) findViewById(R.id.vedio_play);
        ...
        tv_record_delete = findViewById(R.id.tv_record_delete);//播放界面删除按钮
        tv_record_delete.setOnClickListener(this);

        tv_record_choosed = findViewById(R.id.tv_record_choosed);//播放界面确定按钮
        tv_record_choosed.setOnClickListener(this);

        mPlayBottomLayout = findViewById(R.id.rl_play_bottom_layout);//播放界面底部
...
//                        mRecordDelete.setChecked(true);
            ...
            onBackPressed();
        }else if (id == R.id.tv_record_choosed) {
...
//                    mRecordDelete.setVisibility(View.GONE);
    ...
    public void onEncodeComplete() {
        hideProgress();
        //播放已拍好的视频
        playVedio(mMediaObject.getOutputTempTranscodingVideoPath());
